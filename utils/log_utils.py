...
def setup_wandb(
    ...
    tags = [flags.FLAGS.run_group] if flags.FLAGS.run_group is not None else None
    ...
    init_kwargs = dict(
        ...
        entity=entity,
        tags=tags,
        group=flags.FLAGS.run_group,
        job_type=flags.FLAGS.run_job,
    ...
    wandb.login(relogin=True, key=access_key)
    # run = wandb.init(**init_kwargs)

    attempt, wait_time = 1, 60
    while True:
        try:
            run = wandb.init(**init_kwargs)
            return run
        except Error as e:
            print(f"[Wandb Error] {attempt}th-errors: {e}")
            time.sleep(wait_time)
            wait_time = min(wait_time *2, 1200)
            attempt += 1
