# This code is the re-implementation of FBRAC from the 'Flow Q-Learning' paper.
...
class FBRACAgent(flax.struct.PyTreeNode):
    """Flow behavior-regularized actor-critic (FBRAC) agent."""
    ...
    def actor_loss(self, batch, grad_params, rng):
        """Compute the FBRAC actor loss."""
        batch_size, action_dim = batch['actions'].shape
        rng, x_rng, t_rng = jax.random.split(rng, 3)

        # BC loss (flow matching loss).
        ...
        actor_actions = self.update_actions(batch['observations'], params=grad_params, noise=noises)
        ...
        # Total loss.
        actor_loss = bc_flow_loss * self.config['alpha'] + q_loss
    ...
    def update_actions(
        ...
        """Sample actions from the multi-step flow policy with BPTT."""
        actions = noise
        # Euler method.
        for i in range(self.config['flow_steps']):
            t = jnp.full((*observations.shape[:-1], 1), i / self.config['flow_steps'])
            vels = self.network.select('actor_flow')(observations, actions, t, params=params)
    ...
    def sample_actions(
        ...
        """Sample actions from the multi-step flow policy without BPTT."""
